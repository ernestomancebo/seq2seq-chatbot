...
# Question and answer Bucketing size
limit = {
    'maxq': 20,
    'minq': 0,
    'maxa': 20,
...
def filter_data(sequences):
    ...
    filter too long and too short sequences
        return tuple( filtered_q, filtered_a )
    ...
    for i in range(0, len(sequences), 2):
        qlen, alen = len(sequences[i].split(' ')), len(
            sequences[i+1].split(' '))
...
def process_data():
    ...
    print('\n >> Index words')
    idx2w, w2idx, freq_dist = index_(
        qtokenized + atokenized, vocab_size=VOCAB_SIZE)
...
def load_data(PATH=''):
    '''
    Read data control dictionaries - metadata.pkl, idx_q.npy, idx_a.npy. 
        return tuple( metadata, idx_q, idx_a )
