...
def load_vocabulary(metadata):
    ...
    return (word2idx, idx2word, unk_id, pad_id, start_id, end_id, src_vocab_size)


def init_inference(model_, word2idx, idx2word, unk_id, start_id):
    """
    ...
    def inference(seed, top_n):
        ...
        sentence_id = model_(
            inputs=[[seed_id]], seq_length=20, start_token=start_id, top_n=top_n)
        sentence = []
...
if __name__ == "__main__":
    ...
    (word2idx, idx2word, unk_id, pad_id,
     start_id, end_id, vocabulary_size) = load_vocabulary(metadata)
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in tqdm(tl.iterate.minibatches(inputs=trainX, targets=trainY, batch_size=batch_size, shuffle=False),
            ...
            with tf.GradientTape() as tape:
                ...
                loss = cross_entropy_seq_with_mask(
                    logits=output, target_seqs=_target_seqs, input_mask=_target_mask)
        ...
        tl.files.save_npz(model_.all_weights, name=MODEL_NAME)
